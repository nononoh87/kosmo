...
public class Using_Method_Overriding02 {
	...
	public static void main(String[] args) {
		...
		aa.cry();	//aa.cry() : Animal의 cry()호출
		bb.cry();	//bb.cry() : Bird의 cry()호출
		cc.cry();	//cc.cry() : Cat의 cry()호출
		dd.cry();	//dd.cry() : Dog의 cry()호출
		
		System.out.println("=============");
		
		//2. Animal 타입으로 선언 + 자식 클래스로 객체 생성
			//자식의 객체를 생성할때 부모 타입으로 정의후 부모의 메소드를 호출시 오버라이딩된 메소드 출력
			//오버라이딩을 통해서 하나의 타입으로 정의, 배열이나 컬렉션에 저장해서 관리를 쉽게 할수 있다.
		...
		bb1.cry();	//bb1.cry() : Animal의 cry()를 호출 ==> 오버라이딩된 메소드 출력
		cc1.cry();	//cc1.cry() : Animal의 cry()를 호출 ==> 오버라이딩된 메소드 출력
		dd1.cry(); 	//dd1.cry() : Animal의 cry()를 호출 ==> 오버라이딩된 메소드 출력
		...
		int[] arr = new int[] {1,2,3,4};	//배열 선언과 동시에 값을 할당.
		//타입[] 배열변수 =
		...
		for (int i = 0; i < arr3.length; i++) {
			arr3[i].cry();      //Animal의 cry()를 호출시 객체에서 오버라이딩된 메소드가 출력
		}
