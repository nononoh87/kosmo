...
//		-- 부코 클래스(super class)의 필드의 메소드, 이너클래스를 자식 클래스에서 물려준다.
...
//부모 클래스 (Super class) : 자식 클래스의 콩통적인 특징을 가지는 클래스
...
//자식 클래스 : 부모 클래스의 필드와 메소드를 그대로 상속 받는다.
	// -- 자식 클래스만의 특징이 적용된 필드와 메소드를 선언한다.
class Student extends Human {	//Student 클래스 Human 클래스를 상속 받는다.
...
class Worker extends Human {	//직장인
	...
	int workerID;
	void goToWork() {
		System.out.println("직장인이 직장에 갑니다.");
		}
}
